...
const ROL_LABEL = {
    ...
    superadmin:    'Administrador Plataforma',
};
...
const VistaConfiguracion = ({ userId, mostrarAlerta, onPerfilActualizado }) => {
    ...
    const handleCambiarPass = async (e) => {
        ...
        const nuevaPassFinal = nuevaPass.trim();
        if (nuevaPassFinal.length < PASS_MIN) {
            mostrarMensaje('error', `La contraseña debe tener al menos ${PASS_MIN} caracteres.`);
            return;
        }
        if (nuevaPassFinal !== confirmarPass.trim()) {
            mostrarMensaje('error', 'Las contraseñas no coinciden.');
            return;
        }
        if (nuevaPassFinal === passActual) {
        ...
        try {
            ...
            const { error } = await supabase.auth.updateUser({ password: nuevaPassFinal });
            if (error) throw error;
            setPassActual('');
            setNuevaPass('');
            setConfirmarPass('');
