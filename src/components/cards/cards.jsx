...
class VideoStep extends React.Component {
    ...
    // Usamos la API de Wistia aquí para actualizar o pausar el video dinámicamente:
    // https://wistia.com/support/developers/player-api
    componentDidUpdate (prevProps) {
        // Asegura que la API de wistia esté cargada y disponible
        if (!(window.Wistia && window.Wistia.api)) return;

        // Obtiene un identificador del video cargado actualmente
        const video = window.Wistia.api(prevProps.video);

        // Reinicia la fuente de video si se ha elegido un nuevo video de la librería
        if (prevProps.video !== this.props.video) {
            video.replaceWith(this.props.video);
        }

        // Pausa el video si el modal se está reduciendo
...
const Cards = props => {
    ...
    // Las tarjetas de tutorial necesitan calcular sus propios límites de arrastre
    // para permitir arrastrar las tarjetas fuera de los bordes izquierdo, derecho e inferior
    ...
    if (x === 0 && y === 0) {
        ...
        // Las tarjetas más altas son de aproximadamente 320px de alto, y la posición predeterminada está fija
    ...
    return (
        // Superposición personalizada para actuar como el padre delimitador para el arrastrableuso de valores anteriores
        <div
...
export {
    Cards as default,
    // Otros exportados para capacidad de prueba
    ImageStep,
    VideoStep
};
