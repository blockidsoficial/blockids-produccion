...
class UpdatePeripheralStep extends React.Component {
    ...
    async handleSendUpdate () {
        ...
        try {
            const res = await this.props.onSendPeripheralUpdate(progress => {
                // En mi computadora, obtengo una actualización de progreso cada 0.005% más o menos.
                // Renderizar el anillo de progreso es un poco costoso, por lo que filtrar actualizaciones aquí reduce la carga de CPU.
                // Actualizar cada 1% no se ve muy suave, pero 0.5% (1/200) se ve bien para mí.
        ...
        return (<Box className={styles.activityArea}>
            <Box className={styles.scratchLinkHelp}>
                <Box className={styles.scratchLinkHelpStep}>
                    ...
                    />
                    {
                        // Las instrucciones para preparar el periférico para el proceso de actualización varían
                        // según el periférico. ¿Deberemos obtener esto de la extensión de alguna manera?
                    }
                    <FormattedMessage
    ...
    renderResults () {
        ...
        if (this.state.err === null) {
            ...
            // este es un caso especial donde el firmware de comunicación de micro:bit es demasiado antiguo para soportar WebUSB
