...
const getIsFetchingWithoutId = loadingState => (
    // LOADING_VM_FILE_UPLOAD es una captura honoraria, ya que no hay paso de captura para cargas de archivos
...
const reducer = function (state, action) {
    ...
    case DONE_CREATING_NEW:
        // Necesitamos establecer el ID del proyecto ya que acabamos de crear un nuevo proyecto en el servidor.
        // No hay necesidad de cargar, deberemos tener datos ya en vm.
    ...
    case SET_PROJECT_ID:
        // si el projectId no ha cambiado realmente, no hagas nada
        if (state.projectId === action.projectId) {
            return state;
        }
        // si ya mostrábamos un proyecto, y se establece un ID de proyecto diferente, solo trae ese proyecto si
        // el projectId ha cambiado. Esto evita volver a buscar proyectos innecesariamente.
        if (state.loadingState === LoadingState.SHOWING_WITH_ID) {
            ...
            // si ya mostrábamos un proyecto, no hagas la transición al proyecto predeterminado.
            if (action.projectId !== defaultProjectId && action.projectId !== null) {
                return Object.assign({}, state, {
                    loadingState: LoadingState.FETCHING_WITH_ID,
                    projectId: action.projectId
        ...
        } else { // permitir que cualquier otro estado haga la transición a obtener proyecto
            // if setting the default project id, specifically fetch that project
            if (action.projectId === defaultProjectId || action.projectId === null) {
                return Object.assign({}, state, {
                    loadingState: LoadingState.FETCHING_NEW_DEFAULT,
    ...
    case START_UPDATING_BEFORE_CREATING_NEW:
        ...
        return state;
    case START_ERROR:
        // errores fatales: no hay estado de editor correcto para que mostremos
        if ([
        ...
        // errores no fatales: puede mantener mostrando el estado del editor bien
        if ([
        ...
        // error no fatal; el estado a mostrar depende de si el proyecto que mostramos
        // tiene una ID o no
        if (state.loadingState === LoadingState.CREATING_NEW) {
...
const onLoadedProject = (loadingState, canSave, success) => {
    ...
    case LoadingState.LOADING_VM_WITH_ID:
        ...
        // no se pudo cargar el proyecto; solo mantén mostrando el proyecto actual
    ...
    case LoadingState.LOADING_VM_NEW_DEFAULT:
        ...
        // no se pudo cargar el proyecto del proyecto predeterminado; muestra error
