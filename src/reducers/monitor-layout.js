...
// Verifica que el rectángulo formado por los 2 puntos esté bien formado
...
const _moveMonitorRect = function (state, action) {
    ...
    return {
        ...
        // La posición generada por el usuario se guarda
...
// Posición inicial --------------------------
const PADDING = 5;
// TODO: corrige estos números cuando corrijamos https://github.com/LLK/scratch-gui/issues/980
const SCREEN_WIDTH = 400;
const SCREEN_HEIGHT = 300;
const SCREEN_EDGE_BUFFER = 40;

const _rectsIntersect = function (rect1, rect2) {
    // Si un rectángulo está en el lado izquierdo del otro
...
// Necesitamos colocar un monitor con el ancho y alto dados. Devuelve un rectángulo que define dónde debe colocarse.
const getInitialPosition = function (state, monitorId, eltWidth, eltHeight) {
    // Si este monitor fue movido intencionalmente a una cierta posición antes, vuélvelo a esa posición
    ...
    // Intenta todas las posiciones iniciales para el nuevo monitor para encontrar una que no interseque con otras
    const endXs = [0];
    const endYs = [0];
    let lastX = null;
    let lastY = null;
    ...
    endXs.sort((a, b) => a - b);
    endYs.sort((a, b) => a - b);
    // Usaremos el plan B si el monitor no cabe en ninguna parte (demasiado largo o alto)
    let planB = null;
    for (const x of endXs) {
        ...
        for (const y of endYs) {
            ...
            // rect de prueba de intersección que incluye espaciado
            const rect = {
                upperStart: {x, y},
            ...
            // Si el rect se superpone con los extremos de la pantalla
            if (rect.lowerEnd.x > SCREEN_WIDTH || rect.lowerEnd.y > SCREEN_HEIGHT) {
                // Si el rect no está demasiado cerca de estar completamente fuera de la pantalla, establécelo como plan B
                if (!planB &&
    ...
    // Si el monitor es demasiado largo para caber en cualquier lugar, colócalo en el lugar más a la izquierda disponible
    // que interseque el borde derecho o inferior y no esté demasiado cerca del borde.
    if (planB) {
        return planB;
    }

    // Si el plan B falla y no hay ningún lugar razonable para colocarlo, el plan C es colocar el monitor al azar
