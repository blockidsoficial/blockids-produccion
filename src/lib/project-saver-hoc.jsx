    ...
    class ProjectSaverComponent extends React.Component {
        ...
        componentWillMount () {
            if (typeof window === 'object') {
                // Nota: podría ser mejor usar un oyente en lugar de asignar onbeforeunload;
                // pero entonces sería difícil apagar esta escucha en nuestras pruebas
                window.onbeforeunload = e => this.leavePageConfirm(e);
            }

            // Permite al consumidor de GUI pasar una función para recibir un disparador
            // para disparar guardado de miniaturas o proyecto completo.
            // Estas funciones se llaman con null en desmontaje para evitar referencias obsoletas.
        ...
        componentDidUpdate (prevProps) {
            ...
            // ver si debemos "crear" el proyecto actual en el servidor
            //
            // no intentes crear o guardar inmediatamente después de intentar crear
            if (prevProps.isCreatingNew) return;
            // si estamos en condiciones de crear este proyecto, ¡créalo!
            if (this.isShowingCreatable(this.props) && !this.isShowingCreatable(prevProps)) {
                this.props.onCreateProject();
            }

            // ver si debemos guardar/actualizar el proyecto actual en el servidor
            //
            // no intentes guardar inmediatamente después de intentar guardar
            if (prevProps.isUpdating) return;
            // si estamos en condiciones de guardar este proyecto, ¡guárdalo!
        ...
        componentWillUnmount () {
            ...
            // No puedes desactivar el beforeunload porque podría no pertenecerle a este componente
            // es decir, si otro de este componente se montó antes de que este se desmonte
            // lo que sucede cuando pasa de la vista de proyecto a la vista del editor.
            // window.onbeforeunload = undefined; // eslint-disable-line no-undefined
            // Elimina la función de miniaturista de proyecto ya que los componentes se desmontan
            this.props.onSetProjectThumbnailer(null);
            this.props.onSetProjectSaver(null);
        }
        leavePageConfirm (e) {
            if (this.props.projectChanged) {
                // ambos métodos de devolver un valor pueden ser necesarios para la compatibilidad del navegador
        ...
        updateProjectToStorage () {
            ...
            const esProyectoNuevo = !this.props.reduxProjectId || !esUUID(this.props.reduxProjectId);
            const projectIdParaGuardar = esProyectoNuevo ? null : this.props.reduxProjectId;
            return this.storeProject(projectIdParaGuardar, {title: this.props.reduxProjectTitle})
                .then(response => {
                    if (esProyectoNuevo && response && response.id) {
        ...
        createNewProjectToStorage () {
            return this.storeProject(null, {title: this.props.reduxProjectTitle})
                ...
                .catch(err => {
                    // En BLOCKIDS los proyectos se crean desde VistaProyectos antes de abrir
                    // el entorno — si llegamos aquí es modo sandbox sin proyectoId en la URL.
                    // Silenciar el cartel naranja: el usuario ya trabaja con el proyecto en blanco.
                    console.warn('[BLOCKIDS] createNewProjectToStorage ignorado (modo sandbox):', err.message);
                });
        }
        createCopyToStorage () {
        ...
        storeProject (projectId, requestParams) {
            ...
            this.clearAutoSaveTimeout();
            // Serializar el proyecto completo (JSON + activos) como un blob .blockids
            // y guardarlo directo en Supabase, sin pasar por servidor de activos externo.
            return this.props.vm.saveProjectSb3()
                .then(blob => this.props.onUpdateProjectData(projectId, blob, requestParams))
                .then(response => {
        ...
        /**
         * Reporta un evento de telemetría.
         * @param {string} event - uno de `projectWasCreated`, `projectDidLoad`, `projectDidSave`, `projectWasUploaded`
         */
        // TODO: crear un HOC de telemetría y mover esto allá
        reportTelemetryEvent (event) {
            try {
                ...
                // Esto es deliberadamente disparar y olvidar porque una falla
                // al reportar telemetría no debe bloquear el guardado
            }
        }
